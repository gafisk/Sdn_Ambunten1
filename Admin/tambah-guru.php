...
if (isset($_POST['submit'])){
  ...
  if (empty($nip) || empty($nama) || empty($jk) || empty($alamat) || empty($notelp) || empty($notelp)) {
    echo "<script>alert('Kolom Inputan Data Tidak Boleh Kosong!');</script>";
  } else {
    $query = mysqli_query($conn, "INSERT INTO guru VALUES (NULL, '$nip', '$nama', '$alamat', '$jk', '$notelp', '$nip', '$nip', '$kelas' )");
    if ($query) {
      $keterangan = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM kelas WHERE id_kelas = '$kelas'"));
      tambah_log("Menambahkan guru $nama ke " . $keterangan['nama_kelas']);
                ...
                <form method="POST" action="">
                  <div class="card-body">
                    <div class="form-group">
                      ...
                      <input type="text" name="nip" class="form-control" id="ni" placeholder="NIP Guru">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      <label>Alamat</label>
                      <textarea class="form-control" name="alamat" rows="3" placeholder="Alamat Guru"></textarea>
                    </div>
                    <div class="form-group">
